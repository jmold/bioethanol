"""V0.22.0 API extension for the connected discrete-event plant engine.

The steady-state V0.19 engineering model remains authoritative for chemistry,
thermodynamics and utilities. V0.22.0 retains only the time-domain plant payload
...
def _connected_payload(definition: dict, result: dict) -> dict:
    """Build V0.22.0 and retain V0.19 utility readings on the common time grid."""
    ...
    connected["legacy_utility_basis"] = {
        ...
        "utility_basis": "V0.19 time-domain utility model retained during V0.22.0",
    }
    return connected
...
def run_flowsheet_v020(payload: FlowsheetPayload):
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=f"The V0.22.0 model could not calculate this flowsheet: {exc}") from exc
...
def dynamic_plant_v020(payload: FlowsheetPayload):
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail=f"The V0.22.0 connected plant engine could not calculate this flowsheet: {exc}") from exc
