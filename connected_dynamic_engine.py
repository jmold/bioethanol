...
def build_connected_dynamic_simulation(definition:dict,results:dict,timestep_min:int=15,horizon_h:float=168.0)->dict:
    """V0.20.1 direct-transfer connected plant scheduler aligned to the P01-P12 reference route.
    ...
    if len(sections)<3:
        out=dict(legacy)
        out.update({"engine":"Bio-Agri direct-transfer dynamic plant engine","engine_version":"0.20.1","connected_material_transfers":False})
        return out
